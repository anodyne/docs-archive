<h1>The Kitchen Sink</h1>

<div class="row">
	<div class="span4">
		<div class="well">
			<img src="<?php echo Uri::base(false);?>app/views/design/images/nova2/info.png" alt="">
			<h3 class="align-center">General Information</h3>
		</div>
		
		<ul>
			<li><a href="<?php echo Uri::create('nova2/everything/structure');?>">File structure of Nova 2</a></li>
			<li><a href="<?php echo Uri::create('nova2/everything/urls');?>">Nova URLs</a></li>
			<li><a href="<?php echo Uri::create('nova2/everything/versioning');?>">Versioning in Nova</a></li>
			<li><a href="<?php echo Uri::create('nova2/everything/genres');?>">Genres in Nova</a></li>
			<li><a href="<?php echo Uri::create('nova2/everything/license');?>">The Nova License</a></li>
			<li><a href="<?php echo Uri::create('nova2/everything/components');?>">Nova Components</a></li>
		</ul>
	</div>
	
	<div class="span4">
		<div class="well">
			<img src="<?php echo Uri::base(false);?>app/views/design/images/nova2/beaker.png" alt="">
			<h3 class="align-center">Tutorials</h3>
		</div>
		
		<ul>
			<li><a href="<?php echo Uri::create('nova2/everything/configure');?>">Configuring Nova</a></li>
			<li><a href="<?php echo Uri::create('nova2/everything/configure/codeigniter');?>">Configuring CodeIgniter</a></li>
		</ul>
	</div>
	
	<div class="span4">
		<div class="well">
			<img src="<?php echo Uri::base(false);?>app/views/design/images/nova2/history.png" alt="">
			<h3 class="align-center">History</h3>
		</div>
		
		<ul>
			<li><a href="<?php echo Uri::create('nova2/everything/changes/nova');?>">Changes Since Nova 1.2</a></li>
			<li><a href="<?php echo Uri::create('nova2/everything/changelog');?>">Nova 2 Changelog</a></li>
		</ul>
	</div>
</div>