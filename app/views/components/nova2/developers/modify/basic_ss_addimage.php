...
<p>Sometimes though, using a Site Message isn't an option. In those cases, you can use seamless substitution to accomplish the same thing. In order to add an image to a page using seamless substitution, you need to start by uploading the image to either a photo sharing site or your own site. Once you've done that, find the appropriate view file in <code>nova/modules/core/views/_base</code>. For this example, we're going to ignore the fact that we can make the change through Site Messages for the sim page and do it with seamless substitution.</p>

<p>The particular view file you'll need to copy into <code>app/views/_base_override</code> is <code>nova/modules/core/views/_base/main/pages/sim_index.php</code>. How do you know what the right view file is? The <code>main</code> part tells us that this is a page in the main section of the site and the view files are prefixed with the controller and the action. In this case, sim/index. While that's not always the case, view files are named well enough that you'll be able to tell which page it's being used for.</p>

<p>Now that you've copied <code>nova/modules/core/views/_base/main/pages/sim_index.php</code> to <code>app/views/_base_override/main/pages</code> you can edit the version in the <code>_base_override</code> folder. Once you're done making your changes, save the file and upload it to your server. The next time you go to the sim page, you'll see your image wherever you put it in the page. Remember that view files are mostly HTML with some PHP mixed in, so any HTML you want to use can easily be used in view files.</p>
