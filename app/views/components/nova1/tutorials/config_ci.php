...
<ul>
	<li><a href="#" myID="basic" class="shortcut">Basic System Settings</a></li>
	<li><a href="#" myID="adv" class="shortcut">Advanced System Settings</a></li>
	<li><a href="#" myID="email" class="shortcut">System Email Settings</a></li>
	<li><a href="#" myID="ftp" class="shortcut">System FTP Settings</a></li>
</ul>

<h2 class="basic">Basic System Settings</h2>

<a href="#" class="backtop">Back to the Top</a>

<h3>Base URL</h3>

<p>In SMS, it was called the web location variable. In CI, it's called the base URL. CodeIgniter sets the base URL dynamically so you should never have to touch this variable. If you find a situation where the base URL isn't accurate, you can change it in <code>application/config/config.php</code>.</p>

<p class="alert alert-info"><strong>Note:</strong> Take great care when changing the base URL as it can cause Nova to break!</p>

<h3>Index File</h3>

<p>CodeIgniter comes with the ability to rename index.php to whatever you want. This is advantageous for a variety of reasons, namely if you want to run multiple sims from the same directory or if you want to create a splash page at index.php and point to another page for Nova. You can change the index.php file in <code>application/config/config.php</code>. Make sure you have changed the name of the index.php file, then put the changed name in the file.</p>
...
<p>CodeIgniter allows the default character set to be changed to whatever an admin wants. By default, it is set to <dfn>UTF-8</dfn>. For most situations, that should be fine. If you need to change it, you can find the preference in <code>application/config/config.php</code>.</p>

<h3>Error Logging</h3>

<p>CodeIgniter allows admins to change the error logging threshold if they so choose. By default, Nova operates with error reporting turned off, but if you turn it on, you can change the amount of information that is dumped to the error logs. You can change the error logging options from <code>application/config/config.php</code>. In addition, you can change the location where the system logs are stored (if your host has changed it from the default location) and the date format for the error logs.</p>

<h3>Master Time Reference</h3>

<p>Unlike SMS, Nova is a lot smarter with dates and times. By default, Nova will convert all dates to GMT to allow for users to set their individual time zones. You can choose to change this to the server's local time if you want, but doing so will break some of the timezone features in Nova. Admins can change this preference in the <code>application/config/config.php</code> file.</p>

<h3>Database</h3>

<p>CodeIgniter has an extensive list of options for configuring connections to the database. These options are covered in more depth in the <?php echo Html::anchor('nova1/overview/install', 'Installation page');?>. Database connection information can be found in the <code>application/config/database.php</code> file.</p>
...
<h2 class="adv">Advanced System Settings</h2>

<a href="#" class="backtop">Back to the Top</a>

<h3>Auto-Loading</h3>

<p>CodeIgniter comes with a lot of awesome helpers and libraries. By default, Nova auto-loads several of them we use quite often. You can add and remove from these lists in the <code>application/config/autoload.php</code> file.</p>
...
<p>CodeIgniter comes with a powerful hooks system for tapping in to the system during initialization at several different points. Nova does not use hooks, but if you want to develop a plugin or MOD that uses hooks, the feature will need to be enabled. You can enable system hooks by setting the config variable to <dfn>TRUE</dfn>. This config setting can be found in <code>application/config/config.php</code>. More information about system hooks can be found in the <a href="http://codeigniter.com/user_guide/general/hooks.html" target="_blank">CodeIgniter User Guide</a>. If you are using system hooks, you will define them in <code>application/config/hooks.php</code>.</p>
...
<p>By default CodeIgniter uses search-engine friendly segment based URLs: <var>www.your-site.com/who/what/where/</var>. You can optionally enable standard query string based URLs <var>www.your-site.com?who=me&amp;what=something&amp;where=here</var>. The other items let you set the query string "words" that will invoke your controllers and its functions: <var>www.your-site.com/index.php?c=controller&m=function</var>. You can change this in <code>application/config/config.php</code>.</p>
...
<p>CodeIgniter has a feature to allow for Gzipping output for faster page loads. Only enable this feature if you know your server can handling this and are reasonably confident that your users are using browsers that support this feature! The setting can be found in <code>application/config/config.php</code>.</p>

<h3>Rewrite PHP Short Tags</h3>

<p>CodeIgniter has a feature that allows for rewriting PHP short tags on servers that don't have the option turned on in the PHP configuration, eliminating errors and mishandled pages. By default, Nova runs with this disabled as we've gone to great lengths not to use PHP short tags. You can change this setting from <code>application/config/config.php</code>, however, you may see a slight drop in performance by doing so.</p>

<h3>Constants</h3>

<p>CodeIgniter comes with a small group of system constants for working with files and some characters. Those constants can be found in <code>application/config/constants.php</code>.</p>
...
<h2 class="email">System Email Settings</h2>

<a href="#" class="backtop">Back to the Top</a>

<p>Nova uses CodeIgniter's Email class for delivering email from the system to the users of the RPG. The Email class allows for setting a wide array of options for the emails and all those settings can be changed from the configuration file located at <code>application/config/email.php</code>. Below are some of the most common items admins may want to change. For more detailed information about CodeIgniter's Email class, please visit the <a href="http://codeigniter.com/user_guide/libraries/email.html" target="_blank">CodeIgniter User Guide</a>.</p>
...
<h2 class="ftp">System FTP Settings</h2>
...
<p>From the FTP config file located at <code>application/config/ftp.php</code>, you can set your host name, username, password and port for use by CodeIgniter in connecting to your server for advanced operations. If you change settings in this file, make sure the file's permissions on the server are set to an appropriately safe level (664 should be fine).</p>