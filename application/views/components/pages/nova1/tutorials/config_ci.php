...
<p>In SMS, it was called the web location variable. In CI, it's called the base URL. CodeIgniter sets the base URL dynamically so you should never have to touch this variable. If you find a situation where the base URL isn't accurate, you can change it in <samp>application/config/config.php</samp>.</p>
...
<p>CodeIgniter has an extensive list of options for configuring connections to the database. These options are covered in more depth in the <?php echo Html::anchor('nova1/overview/install', 'Installation page');?>. Database connection information can be found in the <samp>application/config/database.php</samp> file.</p>
