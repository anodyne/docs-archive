...
<p>To start, we're going to open <samp>application/controllers/sim.php</samp>. This is the controller that handles all of the pages in the sim section. When you open the file, you'll notice that it's almost empty. The core sim pages are stored in the base controller in the Nova module. Since we're not interested in modifying those, we're going to just add a new controller action to this file after the comment about adding your own methods after it.</p>
...
<p>In order for seamless substitution to work, we created a location class (it used to be a helper in Nova 1) that does all the heavy lifting and figures out where to pull files from. In this case, we're looking for a view that's named <samp>sim_allawards.php</samp> (the .php part is assumed so you don't have to include it). After that, the class is simplying being told where to look and what section it's part of.</p>
...
<p>If you tried to go to your page above, you'd be greeted by a nasty error telling you something is missing. What we're missing is the guts of the page, or the view file. The second part of this is to create your view file. To do that, we're going to create a file called <samp>sim_allawards.php</samp> in <nobr><samp>application/views/_base_override/main/pages</samp></nobr>.</p>
...
&lt;p>&lt;img src="&lt;?php echo base_url();?>application/assets/images/award_image.jpg" />&lt;/p></pre>
...
<p>From here, you can make whatever changes you want to the view file and continue to add awards. Because we've done all this work in the <samp>application</samp> directory, we don't have to worry about losing our changes when we make an update either!</p>